...
class IDSynth(BaseSynth):
    ...
    def __init__(self, args, data_dir, device="cpu"):
        ...
        self.sample_size = (
            args.base_generator.sample_size
        )  # actual input sample size (downsampled if necessary)
    ...
    def read_ground_truth(
        self, idx, img, loc_list, scaling_factor_distances, flip
    ):
        ...
        if S is not None:
            if self.deform_one_hots:
                ...
                Sdef = fast_3D_interp_torch(
                    S, xx2, yy2, zz2, "nearest", self.device
                )
        ...
        if (
            flip
        ):  # only flip subject-robust for output, other params to be flipped in process_sample()
    ...
    def process_sample(
        ...
        if "sr" in self.task:
            ...
            SYN_cerebral = fast_3D_interp_torch(
                SYN_cerebral, xx2, yy2, zz2, "linear"
            )
    ...
    def _getitem_from_id(self, idx):
        ...
        (
            ...
            y2,
            z2,
        ) = self.get_deformation(Gshp)

        # Read in data
        G, S, Sdef, Sdef_OneHot, D, Idef, B = self.read_ground_truth(
            idx,
            Gimg,
            ...
            if sample_i < self.mild_samples:
                ...
                samples.append(
                    self.generate_sample(
                        ...
                        yy2,
                        zz2,
                        **vars(self.args.severe_generator)
                    )
                )
