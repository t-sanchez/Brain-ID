...
class IDSynthEval(Dataset):
    ...
    def __init__(self, args, data_dir, device="cpu"):
        ...
        self.label_list_segmentation = (
            args.base_test_generator.label_list_segmentation_with_csf
        )
        self.n_neutral_labels = (
            args.base_test_generator.n_neutral_labels_with_csf
        )
        self.n_steps_svf_integration = (
            args.base_test_generator.n_steps_svf_integration
        )
        ...
        self.data_augmentation = (
            args.base_test_generator.data_augmentation
        )  # if False, input original image
        self.apply_deformation = (
            args.base_test_generator.apply_deformation
            and self.data_augmentation
        )
        self.nonlinear_transform = (
            args.base_test_generator.nonlinear_transform
            and self.data_augmentation
            and self.apply_deformation
        )
        self.integrate_deformation_fields = (
            args.base_test_generator.integrate_deformation_fields
            and self.nonlinear_transform
        )

        self.apply_gamma_transform = (
            args.base_test_generator.apply_gamma_transform
            and self.data_augmentation
        )
        self.apply_bias_field = (
            args.base_test_generator.apply_bias_field
            and self.data_augmentation
        )
        self.apply_resampling = (
            args.base_test_generator.apply_resampling
            and self.data_augmentation
        )
        self.hyperfine_prob = (
            args.base_test_generator.hyperfine_prob
            if self.apply_resampling
            else 0.0
        )
        self.apply_noises = (
            args.base_test_generator.apply_noises and self.data_augmentation
        )
        ...
        names = glob.glob(os.path.join(data_dir, "*.nii.gz")) + glob.glob(
    ...
    def random_affine_transform(
        self, shp, max_rotation, max_shear, max_scaling
    ):
        rotations = (
        ...
        c2 = torch.tensor(
            (np.array(shp[0:3]) - 1) / 2, dtype=torch.float, device=self.device
        )
    ...
    def random_nonlinear_transform(
        ...
        size_F_small = (
            np.round(nonlin_scale * np.array(shp)).astype(int).tolist()
        )
        if photo_mode:
        ...
        **kwargs
        ...
        if self.data_augmentation:
            ...
            if self.apply_bias_field:
                ...
                size_BF_small = (
                    np.round(bf_scale * np.array(I_shp)).astype(int).tolist()
                )
                if photo_mode:
                ...
                BFsmall = torch.tensor(
                    ...
                    size_BF_small, dtype=torch.float, device=self.device
                )
                BFlog = utils.myzoom_torch(
                    BFsmall, np.array(I_shp) / size_BF_small
                )
                BF = torch.exp(BFlog)
                I_def = I_def * BF
            ...
            if self.apply_resampling:
                ...
                vx = np.arange(
                    delta[0],
                    delta[0] + new_size[0] / factors[0],
                    1 / factors[0],
                )[: new_size[0]]
                vy = np.arange(
                    delta[1],
                    delta[1] + new_size[1] / factors[1],
                    1 / factors[1],
                )[: new_size[1]]
                vz = np.arange(
                    delta[2],
                    delta[2] + new_size[2] / factors[2],
                    1 / factors[2],
                )[: new_size[2]]
                II, JJ, KK = np.meshgrid(
                    vx, vy, vz, sparse=False, indexing="ij"
                )
            ...
            if self.apply_noises:
                ...
                I_def = I_def + noise_std * torch.randn(
                    I_def.shape, dtype=torch.float, device=self.device
                )
                I_def[I_def < 0] = 0
        ...
        if (
            self.apply_bias_field or self.bias_field_prediction
        ) and self.data_augmentation:
    ...
    def random_sampler(self, photo_mode, hyperfine_mode, spac):
        if photo_mode:
            resolution = np.array(
                [self.res_testing_data[0], spac, self.res_testing_data[2]]
            )
            thickness = np.array(
                [self.res_testing_data[0], 0.0001, self.res_testing_data[2]]
            )
        elif hyperfine_mode:
    ...
    def prepare_data(self, image_path):
        ...
        im = torch.tensor(
            np.squeeze(im), dtype=torch.float32, device=self.device
        )
        im, aff = utils.torch_resize(im, aff, 1.0)
        im, aff = utils.align_volume_to_ref(
        ...
        self.c = torch.tensor(
            (np.array(im.shape) - 1) / 2, dtype=torch.float, device=self.device
        )
    ...
    def _getitem_from_id(self, idx):
        ...
        (
            ...
            spac,
            flip,
        ) = self.generate_deformation(shp)

        # Read in data
        Idef = self.read_ground_truth(
            I, [xx2, yy2, zz2, x1, y1, z1, x2, y2, z2]
        )
            ...
            if sample_i < self.mild_samples:
                samples.append(
                    self.generate_sample(
                        ...
                        spac,
                        flip,
                        **vars(self.args.mild_test_generator)
                    )
                )
            else:
                samples.append(
                    self.generate_sample(
                        ...
                        spac,
                        flip,
                        **vars(self.args.severe_test_generator)
                    )
                )
        ...
        subjects = {
            ...
            "aff": aff,
            "loc_idx": loc_idx,
            "shp": torch.tensor(shp).to(self.device),
            "A": A,
            "c": c2,
            "F": F,
            "Fneg": Fneg,
...
class DeformIDSynthEval(IDSynthEval):
    ...
    def _getitem_from_id(self, idx):
        ...
        subjects = {
            "name": os.path.basename(self.names[idx]).split(".nii")[0],
            "image_orig": I_orig[None],
            "aff": aff,
            "loc_idx": loc_idx,
            "shp": torch.tensor(shp),
        }
        return subjects, samples
