...
class BaseSynth(Dataset):
    ...
    def __init__(self, args, data_dir, device="cpu"):
        ...
        self.label_list_segmentation = (
            args.base_generator.label_list_segmentation_with_csf
        )
        self.n_neutral_labels = args.base_generator.n_neutral_labels_with_csf
        self.n_steps_svf_integration = (
            args.base_generator.n_steps_svf_integration
        )

        self.nonlinear_transform = args.base_generator.nonlinear_transform
        self.deform_one_hots = args.base_generator.deform_one_hots
        self.integrate_deformation_fields = (
            args.base_generator.integrate_deformation_fields
        )
        ...
        if os.path.exists(self.seg_dir) is False:
            print(
                "Directory with target segmentations not found; target segmentations will not be generated"
            )
            self.seg_dir = None
        if os.path.exists(self.dist_dir) is False:
            print(
                "Directory with distance maps not found; distance maps will not be generated"
            )
            self.dist_dir = None
        if os.path.exists(self.im_dir) is False:
            print(
                "Directory with real images not found; real images will not be generated"
            )
            self.im_dir = None
        if os.path.exists(self.bag_dir) is False:
            print(
                "Directory with distance maps for bag simulation not found; fake bags will not be generated"
            )
            self.bag_dir = None
        ...
        if self.produce_surfaces is False:
            ...
            if os.path.exists(self.surface_dir) is False:
                raise Exception(
                    "User is asking for surfaces but directory with surface files was not found!"
                )
            if self.integrate_deformation_fields is False:
                raise Exception(
                    "Using surfaces requires integrating deformation fields; you need to switch on integrate_deformation_fields option"
                )

        names = glob.glob(os.path.join(self.gen_dir, "*.nii.gz")) + glob.glob(
            os.path.join(self.gen_dir, "*.nii")
        )
        print(self.gen_dir, names)
        ...
        self.onehotmatrix = torch.eye(
            n_labels, dtype=torch.float, device=self.device
        )

        nlat = int((n_labels - self.n_neutral_labels) / 2.0)
        self.vflip = np.concatenate(
            [
                np.array(range(self.n_neutral_labels)),
                np.array(range(self.n_neutral_labels + nlat, n_labels)),
                np.array(
                    range(self.n_neutral_labels, self.n_neutral_labels + nlat)
                ),
            ]
        )
    ...
    def random_affine_transform(
        ...
        scaling_factor_distances = (
            np.prod(scalings) ** 0.33333333333
    ...
    def generate_deformation(self, photo_mode, spac, Gshp, random_shift=True):
        ...
        scaling_factor_distances, A, c2 = self.random_affine_transform(
            Gshp,
            self.max_rotation,
            self.max_shear,
            self.max_scaling,
        ...
        if self.nonlinear_transform:
            F, _ = self.random_nonlinear_transform(
                photo_mode,
                spac,
    ...
    def read_data(
        ...
        G = torch.squeeze(
            torch.tensor(
                img.get_fdata()[x1:x2, y1:y2, z1:z2].astype(float),
                dtype=torch.float,
                device=self.device,
            )
        )
        ...
        if self.seg_dir is not None:
            Simg = nib.load(
                os.path.join(self.seg_dir, os.path.basename(self.names[idx]))
            )
            S = torch.squeeze(
                torch.tensor(
                    Simg.get_fdata()[x1:x2, y1:y2, z1:z2].astype(int),
                    dtype=torch.int,
                    device=self.device,
                )
            )
        if self.dist_dir is not None:
            Dimg = nib.load(
                os.path.join(self.dist_dir, os.path.basename(self.names[idx]))
            )
            D = torch.squeeze(
                torch.tensor(
                    Dimg.get_fdata()[x1:x2, y1:y2, z1:z2].astype(float),
                    dtype=torch.float,
                    device=self.device,
                )
            )
        ...
        if self.im_dir is not None:
            Iimg = nib.load(
                os.path.join(self.im_dir, os.path.basename(self.names[idx]))
            )
            I = torch.squeeze(
                torch.tensor(
                    Iimg.get_fdata()[x1:x2, y1:y2, z1:z2].astype(float),
                    dtype=torch.float,
                    device=self.device,
                )
            )
        ...
        if self.bag_dir is not None:
            Bimg = nib.load(
                os.path.join(self.bag_dir, os.path.basename(self.names[idx]))
            )
            B = torch.squeeze(
                torch.tensor(
                    Bimg.get_fdata()[x1:x2, y1:y2, z1:z2].astype(float),
                    dtype=torch.float,
                    device=self.device,
                )
            )
        ...
        if photo_mode or (np.random.rand() < exvixo_prob):
            ...
            if photo_mode:
                ...
                bag_scale = bag_scale_min + np.random.rand(1) * (
                    bag_scale_max - bag_scale_min
                )
                size_TH_small = (
                    np.round(bag_scale * np.array(G.shape))
                    .astype(int)
                    .tolist()
                )
                bag_tness = torch.tensor(
                    np.sort(1.0 + 20 * np.random.rand(2)),
                    dtype=torch.float,
                    device=self.device,
                )
                THsmall = bag_tness[0] + (
                    bag_tness[1] - bag_tness[0]
                ) * torch.rand(
                    size_TH_small, dtype=torch.float, device=self.device
                )
    ...
    def process_sample(
        ...
        SYN = mus[Gr] + sigmas[Gr] * torch.randn(
            Gr.shape, dtype=torch.float, device=self.device
        )
        ...
        if self.pv:
            ...
            pw = (Gv <= 3) * (3 - Gv)
            isv += pw * mus[2] + pw * sigmas[2] * torch.randn(
                Gv.shape, dtype=torch.float, device=self.device
            )
            pg = (Gv <= 3) * (Gv - 2) + (Gv > 3) * (4 - Gv)
            isv += pg * mus[3] + pg * sigmas[3] * torch.randn(
                Gv.shape, dtype=torch.float, device=self.device
            )
            pcsf = (Gv >= 3) * (Gv - 3)
            isv += pcsf * mus[4] + pcsf * sigmas[4] * torch.randn(
                Gv.shape, dtype=torch.float, device=self.device
            )
            SYN[mask] = isv
        ...
        Sdef_OneHot = Ddef = Idef = Pdef = 0.0
        Sdef = fast_3D_interp_torch(S, xx2, yy2, zz2, "nearest")
        if S is not None:
            if self.deform_one_hots:
                Sonehot = self.onehotmatrix[self.lut[S.long()]]
                Sdef_OneHot = fast_3D_interp_torch(
                    Sonehot, xx2, yy2, zz2, "linear"
                )
            else:
                Sdef_OneHot = self.onehotmatrix[self.lut[Sdef.long()]]
        if D is not None:
            Ddef = fast_3D_interp_torch(
                D, xx2, yy2, zz2, "linear", default_value_linear=torch.max(D)
            )
        if I is not None:
        ...
        # Gamma transform
        gamma = torch.tensor(
            np.exp(gamma_std * np.random.randn(1)[0]),
            dtype=float,
            device=self.device,
        )
        ...
        # Bias field
        bf_scale = bf_scale_min + np.random.rand(1) * (
            bf_scale_max - bf_scale_min
        )
        size_BF_small = (
            np.round(bf_scale * np.array(self.size)).astype(int).tolist()
        )
        if photo_mode:
            size_BF_small[1] = np.round(self.size[1] / spac).astype(int)
        BFsmall = torch.tensor(
            bf_std_min + (bf_std_max - bf_std_min) * np.random.rand(1),
            dtype=torch.float,
            device=self.device,
        ...
        stds = (
            ...
            * np.log(5)
            / np.pi
            * thickness
            / self.res_training_data
        )
        stds[thickness <= self.res_training_data] = (
            0.0  # no blur if thickness is equal to the resolution of the training data
        )
        SYN_blur = gaussian_blur_3d(SYN_bf, stds, self.device)
        new_size = (
            np.array(self.size) * self.res_training_data / resolution
        ).astype(int)
        ...
        noise_std = torch.tensor(
            noise_std_min
            + (noise_std_max - noise_std_min) * np.random.rand(1),
            dtype=torch.float,
            device=self.device,
        )
        SYN_noisy = SYN_small + noise_std * torch.randn(
            SYN_small.shape, dtype=torch.float, device=self.device
        )
        SYN_noisy[SYN_noisy < 0] = 0

        # Back to original resolution
        if self.bspline_zooming:
            SYN_resized = interpol.resize(
                SYN_noisy,
                shape=self.size,
                anchor="edge",
                interpolation=3,
    ...
    def get_contrast(self, photo_mode):
        ...
        mus = 25 + 200 * torch.rand(
            10000, dtype=torch.float, device=self.device
        )
        sigmas = 5 + 20 * torch.rand(
            10000, dtype=torch.float, device=self.device
        )
        if (
            photo_mode or np.random.rand(1) < 0.5
    ...
    def random_sampler(self, photo_mode, hyperfine_mode, spac):
        if photo_mode:
            resolution = np.array(
                [self.res_training_data[0], spac, self.res_training_data[2]]
            )
            thickness = np.array(
                [self.res_training_data[0], 0.0001, self.res_training_data[2]]
            )
        elif hyperfine_mode:
            resolution = np.array([1.6, 1.6, 5.0])
            thickness = np.array([1.6, 1.6, 5.0])
        else:
    ...
    def __getitem__(self, idx):
        ...
        # sample affine deformation
        scaling_factor_distances, A, c2 = self.random_affine_transform(
            Gshp, self.max_rotation, self.max_shear, self.max_scaling
        )

        # sample nonlinear deformation
        F, Fneg = self.random_nonlinear_transform(
            photo_mode,
            spac,
            self.nonlin_scale_min,
            self.nonlin_scale_max,
        ...
        if self.produce_surfaces:
            Vlw, Flw, Vrw, Frw, Vlp, Flp, Vrp, Frp = self.generate_surface(
                idx, Fneg, A, c2
            )
        else:
        ...
        resolution, thickness = self.random_sampler(
            photo_mode, hyperfine_mode, spac
        )
