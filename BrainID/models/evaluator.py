...
class Evaluator:
    ...
    def get_normalized_l2(self, metric_name, output, target, *kwargs):
        ...
        l2 = 0.0 + torch.sqrt(
            torch.sum((w * output - target) ** 2)
            / (torch.sum(target**2) + 1e-7)
        )
    ...
    def get_l1(self, metric_name, output, target, nonzero_only=False, *kwargs):
        if nonzero_only:  # compute only within face_aware_region #
            ...
            l1 = (abs(target - output) * nonzero_mask).sum(
                dim=0
            ) / nonzero_mask.sum(dim=0)
        else:
    ...
    def get_psnr(self, metric_name, output, target, *kwargs):
        ...
        if mse == 0:
            ...
            psnr = 20 * math.log10(
                np.max(target.cpu().numpy()) / math.sqrt(mse)
            )
    ...
    def get_ms_ssim(self, metric_name, output, target, *kwargs):
        ...
        try:
            ...
            print(
                "Error in MS-SSIM: Image too small for Multi-scale SSIM computation. Skipping..."
            )
            return {metric_name: float("nan")}

    def get_score(self, metric_name, output, target, **kwargs):
        assert (
            metric_name in self.metric_map
        ), f"do you really want to compute {metric_name} metric?"
        return self.metric_map[metric_name](
            metric_name, output, target, **kwargs
        )
    ...
    def eval(
        ...
        **kwargs,
        ...
        pred = MRIread(
            pred_path,
            im_only=True,
            dtype="int" if "label" in os.path.basename(pred_path) else "float",
        )
        target, aff = MRIread(
            target_path,
            im_only=False,
            dtype=(
                "int" if "label" in os.path.basename(target_path) else "float"
            ),
        )
        ...
        if is_seg:
            ...
            pred = torch.tensor(
                np.squeeze(pred), dtype=torch.float32, device=self.device
            )
            target = torch.tensor(
                np.squeeze(target), dtype=torch.float32, device=self.device
            )
