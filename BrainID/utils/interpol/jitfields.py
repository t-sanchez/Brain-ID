...
def grid_count(
    ...
    return jitfields.count(
        grid, shape, order=interpolation, bound=bound, extrapolate=extrapolate
    )
...
def resize(
    ...
    **kwargs
    ...
    ndim = max(
        len(make_list(factor or [])),
        len(make_list(shape or [])),
        len(make_list(anchor or [])),
    ) or (image.dim() - 2)
