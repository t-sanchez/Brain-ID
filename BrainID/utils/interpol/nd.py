...
@torch.jit.script
def inbounds_mask(
    extrapolate: int, grid, shape: List[int]
) -> Optional[Tensor]:
    ...
    if extrapolate in (0, 2):  # no / hist
        ...
        if extrapolate == 2:
            threshold = 0.5 + tiny
        mask = torch.ones(
            grid.shape[:-1], dtype=torch.bool, device=grid.device
        )
...
@torch.jit.script
def pull(
    inp, grid, bound: List[Bound], spline: List[Spline], extrapolate: int = 1
):
    """
    ...
    weights, _, _, coords, signs = get_weights(
        grid, bound, spline, shape, False, False
    )

    # initialize
    out = torch.zeros(
        [batch, channel, grid.shape[1]], dtype=inp.dtype, device=inp.device
    )

    # iterate across nodes/corners
    range_nodes = [
        torch.as_tensor([d for d in range(n)])
    ...
    for nodes in all_nodes:
        ...
        # apply sign
        sign0: List[Optional[Tensor]] = [
            sgn[n] for sgn, n in zip(signs, nodes)
        ]
...
def push(
    ...
    # initialize
    out = torch.zeros(
        [batch, channel, list_prod_int(shape)],
        dtype=inp.dtype,
        device=inp.device,
    )
...
def grad(
    ...
    # precompute weights along each dimension
    weights, grads, _, coords, signs = get_weights(
        grid, bound, spline, shape, grad=True
    )

    # initialize
    out = torch.zeros(
        [batch, channel, grid.shape[1], dim],
        dtype=inp.dtype,
        device=inp.device,
    )
    ...
    for nodes in all_nodes:
        ...
        for d in range(dim):
            out1 = out0.clone()
            # apply weights
            for dd, (weight, grad1, n) in enumerate(
                zip(weights, grads, nodes)
            ):
                if d == dd:
                    grad11 = grad1[n]
...
def hess(
    ...
    # precompute weights along each dimension
    weights, grads, hesss, coords, signs = get_weights(
        grid, bound, spline, shape, grad=True, hess=True
    )

    # initialize
    out = torch.zeros(
        [batch, channel, grid.shape[1], dim, dim],
        dtype=inp.dtype,
        device=inp.device,
    )
    ...
    for nodes in all_nodes:
        ...
        for d in range(dim):
            ...
            # apply weights
            for dd, (weight, hess1, n) in enumerate(
                zip(weights, hesss, nodes)
            ):
                if d == dd:
                    hess11 = hess1[n]
