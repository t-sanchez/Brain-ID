...
def pad_shape(array_list):
    ...
    for array in array_list:
        max_shape = [
            max(max_shape[dim], array.shape[dim])
            for dim in range(len(max_shape))
        ]
    pad_array_list = []
    for array in array_list:
        start = [
            (max_shape[dim] - array.shape[dim]) // 2
            for dim in range(len(max_shape))
        ]
        if len(start) == 2:
            pad_array = np.zeros((max_shape[0], max_shape[1]))
            pad_array[
                start[0] : start[0] + array.shape[0],
                start[1] : start[1] + array.shape[1],
            ] = array
        elif len(start) == 3:
...
class BaseVisualizer(object):
    ...
    def prepare_for_png(self, array, normalize=False):  # (s, r, c, *)
        ...
        row = array[:, :: self.vis_spacing[1]].transpose((1, 0, 2, 3))[
            :, ::-1
        ]  # (s, r', c, *) -> (r', s, c, *)
        col = array[:, :, :: self.vis_spacing[2]].transpose((2, 0, 1, 3))[
            :, ::-1
        ]  # (s, r, c', *) -> (c', s, r, *)
...
class FeatVisualizer(BaseVisualizer):
    ...
    def visualize_all_multi(
        self, subjects, multi_inputs, multi_outputs, out_dir
    ):
        """
        ...
        if "reg" in self.task:
            ...
            src_inputs = [
                x["source"] for x in multi_inputs
            ]  # n_samples * (b, d, s, r, c)
            src_features = [x["feat"]["src_feat"] for x in multi_outputs]
            self.visualize_all_multi_features(
                subjects["name"], src_features, src_inputs, out_dir
            )

            tgt_inputs = [
                x["target"] for x in multi_inputs
            ]  # n_samples * (b, d, s, r, c)
            tgt_features = [x["feat"]["tgt_feat"] for x in multi_outputs]
            self.visualize_all_multi_features(
                subjects["target_name"], tgt_features, tgt_inputs, out_dir
            )
        else:
            multi_inputs = [
                x["input"] for x in multi_inputs
            ]  # n_samples * (b, d, s, r, c)
            multi_features = [x["feat"] for x in multi_outputs]
            self.visualize_all_multi_features(
                subjects["name"], multi_features, multi_inputs, out_dir
            )

    def visualize_all_multi_features(
        self, names, multi_features, multi_inputs, out_dir
    ):
        ...
        multi_inputs_reorg = (
            []
        )  # batch_size * [ n_samples * (channel_dim, *img_shp) ]
        multi_features_reorg = (
            []
        )  # batch_size * [ n_samples * [ n_levels * (channel_dim, *img_shp) ] ]
        for i_name, _ in enumerate(names):
            multi_features_reorg.append(
                [
                    [
                        multi_features[i_sample][i_level][i_name]
                        for i_level in range(n_levels)
                    ]
                    for i_sample in range(n_samples)
                ]
            )
            multi_inputs_reorg.append(
                [
                    multi_inputs[i_sample][i_name]
                    for i_sample in range(n_samples)
                ]
            )
        ...
        for i_name, name in enumerate(names):
            ...
            for i_sample in range(n_samples):
                ...
                curr_input = (
                    inputs[i_sample].data.cpu().numpy()
                )  # ( d=1, s, r, c)
                curr_input = self.prepare_for_itk(
                    curr_input.transpose(3, 2, 1, 0)
                )  # (d, x, y, z) -> (z, y, x, d)

                curr_feat = features[
                    i_sample
                ]  # n_levels * (channel_dim, s, r, c)
                curr_level_feats = []
                ...
                for l in range(n_levels):
                    ...
                    sub_idx = even_sample(
                        curr_level_feat.shape[0], self.feat_vis_num
                    )
                    curr_level_feat = torch.stack(
                        [curr_level_feat[idx] for idx in sub_idx], dim=0
                    )  # (sub_channel_dim, s, r, c)

                    curr_level_feat = match_shape(
                        curr_level_feat, list(curr_input.shape[:-1])
                    )
                    curr_level_feats.append(
                        self.prepare_for_itk(
                            (
                                curr_level_feat.data.cpu()
                                .numpy()
                                .transpose((3, 2, 1, 0))
                            )
                        )
                ...
                for l, result in enumerate(all_results):  # n_level * (r, c)
                    gap = np.zeros_like(
                        result[
                            ...
                            : int(
                                result.shape[1]
                                / (curr_input.shape[0] / self.vis_spacing[0])
                            ),
                        ]
                    )
            ...
            for l in all_sample_results.keys():
                curr_level_all_sample_feats = np.concatenate(
                    list(all_sample_results[l][:-1]), axis=1
                )  # (s, n_samples * c)
                Image.fromarray(curr_level_all_sample_feats).save(
                    os.path.join(
                        make_dir(os.path.join(out_dir, name)),
                        name + "_feat_l%s.png" % str(l),
                    )
                )
    ...
    def visualize_all(self, names, inputs, features):
        ...
        for i_name, name in enumerate(names):
            curr_input = self.prepare_for_itk(
                inputs[i_name].transpose((3, 2, 1, 0))
            )  # (d, x, y, z) -> (z, y, x, d)
            curr_level_feats = []
            for l in range(n_levels):
                ...
                curr_feat = torch.stack(
                    [curr_feat[idx] for idx in sub_idx], dim=0
                )  # (sub_channel_dim, s, r, c)

                curr_feat = match_shape(curr_feat, list(curr_input.shape[:-1]))
                curr_level_feats.append(
                    self.prepare_for_itk(
                        (curr_feat.data.cpu().numpy().transpose((3, 2, 1, 0)))
                    )
                )
    ...
    def gather(self, input, feats):
        ...
        input_slc = self.prepare_for_png(input, normalize=False)[0][
            ..., 0
        ]  # (sub_s, r, c)
        all_images = []
        for l, feat in enumerate(feats):
            ...
            slc_feat = normalize(
                feat[:: self.vis_spacing[0]].transpose(3, 0, 1, 2), channel=1
            )  # (sub_s, r, c, sub_channel_dim) -> (sub_channel_dim, sub_s, r, c)
            slc_images = [input_slc, np.zeros_like(input_slc)] + list(
                slc_feat
            )  # (1 + 1 + s', r, c *)
...
class TaskVisualizer(BaseVisualizer):
    ...
    def visualize_all(
        ...
        for output_name in output_names:
            if output_name in outputs[0].keys():
                out_images[output_name] = [
                    x[output_name].data.cpu().numpy() for x in outputs
                ]  # n_samples * (b, d, s, r, c)
        ...
        for i, name in enumerate(names):
            ...
            if Idefs is not None:
                curr_Idef = [
                    self.prepare_for_itk(
                        Idefs[i_sample][i].transpose((3, 2, 1, 0))
                    )
                    for i_sample in range(n_samples)
                ]  # n_samples * (d, x, y, z) -> n_samples (z, y, x, d)
            ...
            if len(out_images) > 0:
                ...
                if not self.subject_robust:
                    for target_name in target_names:
                        if target_name in samples[0]:
                            curr_target[target_name] = [
                                self.prepare_for_itk(
                                    samples[i_sample][target_name][i]
                                    .data.cpu()
                                    .numpy()
                                    .transpose((3, 2, 1, 0))
                                )
                ...
                curr_outputs = {}
                for output_name in output_names:
                    curr_outputs[output_name] = [
                        self.prepare_for_itk(
                            out_images[output_name][i_sample][i].transpose(
                                (3, 2, 1, 0)
                            )
                        )
                        for i_sample in range(n_samples)
                ...
                for i_sample, curr_input in enumerate(curr_inputs):
                    ...
                    for target_name in target_names:
                        if target_name in curr_target:
                            if (
                                not self.subject_robust
                                or "bias_field" in target_name
                            ):
                                target_list.append(
                                    curr_target[target_name][i_sample]
                                )
                            else:
                    ...
                    all_image = self.gather(
                        target_list, output_list
                    )  # (row, col)
                    all_images.append(all_image)  # n_sample * (row, col)
                all_images = np.concatenate(all_images, axis=1).astype(
                    np.uint8
                )  # (row, n_sample * col)
    ...
    def visualize_sample(self, name, input, out_dir, postfix="_input"):
        ...
        for i_sample in range(n_samples):
            input_slc, input_row, input_col = self.prepare_for_png(
                input[i_sample], normalize=False
            )
        ...
        Image.fromarray(all_image[:, :, 0]).save(
            os.path.join(out_dir, name + "_all" + postfix + ".png")
        )  # grey scale image last channel == 1
        return
    ...
    def gather(self, target_list=[], output_list=[]):
        ...
        for add_target in target_list:
            add_target_slc, add_target_row, add_target_col = (
                self.prepare_for_png(add_target, normalize=False)
            )
        ...
        for add_output in output_list:
            add_output_slc, add_output_row, add_output_col = (
                self.prepare_for_png(add_output, normalize=False)
            )
