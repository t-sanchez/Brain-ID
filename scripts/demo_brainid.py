...
imgs = {
    ...
    "T2w": "/media/tsanchez/tsanchez_data/data/data/derivatives/nesvor/nesvor/sub-chuv006/ses-01/anat/sub-chuv006_ses-01_res-1p1_run-1_T2w.nii.gz",
}


input_modality = "T2w"  # Try different input modalities!
...
viewVolume(
    mprage,
    aff,
    names=["out_mprage_from_%s" % input_modality],
    save_dir=make_dir("outs"),
)
...
for i in range(num_plot_feats):
    viewVolume(
        ...
        aff,
        names=["feat-%d" % (i + 1)],
        save_dir=make_dir("outs/feats-%s" % input_modality),
    )
