...
def generate(args):
    ...
    # ============ preparing data ... ============
    dataset = build_dataset_single(
        vars(args.dataset_name)[args.split],
        split=args.split,
        args=args,
        device=device,
    )
    ...
    for itr in range(args.test_itr_limit):
        ...
        print(
            ...
            % (itr, len(dataset), dataset.names[itr])
        )

        for i_deform in range(args.num_deformations):
            def_save_dir = utils.make_dir(
                os.path.join(save_dir, "deform-#%s" % i_deform)
            )
            ...
            for i_sample, sample in enumerate(samples):
                ...
                sample_save_dir = utils.make_dir(
                    os.path.join(def_save_dir, "sample-#%s" % i_sample)
                )

                if "input" in sample:
                    utils.viewVolume(
                        map_back_orig(sample["input"], loc_idx, shp),
                        aff,
                        names=["input"],
                ...
                if "sr" in args.task:
                    utils.viewVolume(
                        map_back_orig(sample["orig"], loc_idx, shp),
                        aff,
                        names=["high_field"],
                        save_dir=sample_save_dir,
                    )
            ...
            if "input" in sample:
                inputs = [
                    x["input"].data.cpu().numpy() for x in samples
                ]  # n_samples * (b, d, s, r, c)
                curr_inputs = [
                    vis.prepare_for_itk(
                        inputs[i_sample].transpose((3, 2, 1, 0))
                    )
                    for i_sample in range(len(samples))
                ]  # n_samples * (d, x, y, z) -> n_samples (z, y, x, d)
                vis.visualize_sample(
                    subj_name, curr_inputs, def_save_dir, postfix="_inputs"
                )
