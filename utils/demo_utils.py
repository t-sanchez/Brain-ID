...
def center_crop(img, win_size=[220, 220, 220]):
    ...
    if (
        orig_shp[0] > win_size[0]
        or orig_shp[1] > win_size[1]
        or orig_shp[2] > win_size[2]
    ):
        crop_start = [
            max((orig_shp[i] - win_size[i]), 0) // 2 for i in range(3)
        ]
        return (
            img[
                :,
                :,
                crop_start[0] : crop_start[0] + win_size[0],
...
@torch.no_grad()
def get_feature(
    inputs, ckp_path, feature_only=False, device="cpu", additional_cfg=None
):

    cfgs = [default_cfg_file, default_data_file, submit_cfg_file]
    if additional_cfg is not None:
        additional_cfg = (
            additional_cfg
            if isinstance(additional_cfg, list)
            else [additional_cfg]
        )
        cfgs += additional_cfg
    # inputs: (batch_size, 1, s, r, c)
    args = utils.preprocess_cfg(cfgs)
